...
c         flush (6) ! commented out by Marti
...
c v0 volume at pr,tr; nn number of atoms; gam0 Gr\FCneisen parameter;
...
C            write (*,1010) char8,x,fname(i),xmng(i,j,k),xmxg(i,j,k)

         else

C            write (*,1020) j,k,x,fname(i),xmng(i,j,k),xmxg(i,j,k)

         end if

         if (refine) then
C            write (*,1000) 'the *.arf file and restart MEEMUM.'
         else 
C            write (*,1000) 'the solution model file and restart MEEMUM.'
         end if 
